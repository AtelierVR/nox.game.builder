	...
	public static class ExternalBuilder {
		...
		private static async UniTaskVoid RunBuildModAsync() {
			try {
				...
				// Ensure Assets/link.xml is generated before the player build
				// (ModLinkerHelper lives in nox.editor, call via reflection to avoid asmdef coupling)
				EnsureLinkXml();
		...
		private static void EnsureLinkXml() {
			...
			try {
				...
				Logger.Log("Assets/link.xml generated by ModLinkerHelper.", tag: nameof(ExternalBuilder));
			} catch (Exception e) {
				Logger.LogWarning($"Failed to generate link.xml: {e.Message}", tag: nameof(ExternalBuilder));
			}
#else
			Logger.LogWarning("nox.editor not available - link.xml may be missing.", tag: nameof(ExternalBuilder));
#endif
		}
